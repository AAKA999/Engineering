     ...
     echo "hello" ;
                if($value['break_fast']=="a") {
                   ...
                   for($i = 0;$i< count($number_of_doses);$i++) { //number of doses equL TO NUMBER of enrty in the shedule table
                    if($count_ab == 1) {
                     ...
                     $time = date("H:i:s",strtotime("+420 minutes", $date));
                     //echo $time."hell";
                   }
                   if($count_ab == 0) {
                      $break_fast = $value['breakfast_time'];
               ...
               $number_of_doses = $value['no_of_dose'];
                   ...
                   for($i = 0;$i< count($number_of_doses);$i++) { //number of doses equL TO NUMBER of enrty in the shedule table
                    ...
                    $time = date("H:i:s",strtotime("+120 minutes", $date));
                  }
                  if($count_ad == 0){
                     $dinner_time = $value['dinner_time'];
                     $date = strtotime(date('$dinner_time'));
               ...
               $number_of_doses = $value['no_of_dose'];
                   ...
                   for($i = 0;$i< count($number_of_doses);$i++) { //number of doses equL TO NUMBER of enrty in the shedule table
                     ...
                     if($count_bb ==' 0')  {
                       ...
                       //echo $break_fast;
                       $date = strtotime(date('$break_fast'));
                       //echo date('$break_fast');
                      $time = date("H:i:s",strtotime("+180 minutes", $date));
					  $count_bb++ ;
          ...
          if($value['break_fast']=="a") {
            $number_of_doses = $value['no_of_dose'];
                for($i = 0;$i< count($number_of_doses);$i++) { //number of doses equL TO NUMBER of enrty in the shedule table
                    ...
                    $time = $value['breakfast_time'];
                     //$date = strtotime(date('$dinner_time'));
                     //$time = date("H:i:s",strtotime("+0 minutes", $date));               $sql = "insert into schedule (patient_id,latest_med_name,date,time) values ('".$value['patient_id']."','".$value['medicene']."','".$value['day']."','".$time."')";
                 mysqli_query($con,$sql);
